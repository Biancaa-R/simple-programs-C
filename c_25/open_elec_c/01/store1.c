...
void insert_into(){
    ...
    if (already+num<50){
        for (int i=0;i<num;i++){
            //struct store* s1;
            array1[i] = (struct store*)malloc(sizeof(struct store));
            printf("Enter the product id of the product\n");
            scanf("%d",&array1[already-1+i]->PID);
            printf("Enter the name of the product\n");
            scanf("%s",&array1[already-1+i]->name);
            printf("Enter the price of the product\n");
            scanf("%f",&array1[already-1+i]->price);
            printf("Enter the qty\n");
            scanf("%d",&array1[already-1+i]->qty);
